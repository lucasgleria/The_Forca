...
### Desenvolvedor B 
- **Foco:** Configuração do projeto, Frontend completo (HTML, CSS/Tailwind/Sass, JavaScript complexo), Backend (FastAPI, design de API, persistência de dados), integração e processo de build.
...
- **Tarefa B1.1.1:** Criar repositório Git (Ex: GitHub/GitLab/Bitbucket) e configurar o `.gitignore` para ignorar arquivos de ambiente, caches, etc.
- **Tarefa B1.1.2:** Criar ambiente virtual Python (`venv`) no diretório raiz do projeto e ativá-lo.
- **Tarefa B1.1.3:** Instalar as dependências iniciais: `pip install fastapi uvicorn`.
- **Tarefa B1.1.4:** Criar o arquivo `requirements.txt` com as dependências instaladas (`pip freeze > requirements.txt`).
- **Tarefa B1.1.5:** Criar a estrutura inicial de pastas para o projeto.
...
- **Tarefa B1.3.2:** Configurar a aplicação FastAPI inicial e o servidor Uvicorn para rodar em `localhost:8000`.  
...
- **Tarefa B2.1.1:** Criar o arquivo `src/frontend/index.html`.
- **Tarefa B2.1.2:** Adicionar a estrutura básica do HTML5 (`<!DOCTYPE html>`, `<html>`, `<head>`, `<body>`). Incluir meta tag para responsividade (`<meta name="viewport" content="width=device-width, initial-scale=1.0">`).
- **Tarefa B2.1.3:** Linkar o arquivo CSS (`style.css` - será gerado pelo Sass/Tailwind) no `<head>` e o JavaScript (`app.js`) antes do fechamento da tag `</body>`.
- **Tarefa B2.1.4:** Adicionar os "containers" para as três telas principais:  
  ```html
...
- **Tarefa B2.2.1:** Configurar o ambiente de frontend para usar Tailwind CSS e Sass.
  ...
  - No diretório `src/frontend`, inicializar npm (`npm init -y`).
  - Instalar Tailwind CSS e PostCSS:  
    ```
  ...
  - Configurar `tailwind.config.js` para escanear arquivos HTML e JS por classes Tailwind:  
    ...
    content: ["./*.html", "./*.js"]
    ```
  - Criar o arquivo `src/frontend/style.scss` e adicionar as diretivas Tailwind:
    ```scss
    @tailwind base;
    @tailwind components;
    @tailwind utilities;
...
- **Tarefa B2.3.1:** Criar o arquivo `src/frontend/app.js`.
...
- **Tarefa B4.1.2:** Criar o módulo `src/backend/ranking_manager.py` com as seguintes funções:
